...
export class ImportService {
  constructor(@Inject('PLUGIN_REGISTRY') private readonly pluginRegistry: PluginRegistry) {}
  ...
  async importFromUrl(url: string, format?: 'json' | 'yaml' | 'xml'): Promise<any> {
    try {
      ...
      const parser = this.pluginRegistry
        .getAllParsers()
        .find((p) => p.getSupportedFormats().includes(detectedFormat!));
