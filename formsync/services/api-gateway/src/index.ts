...
app.use('/schema', proxy(SERVICES.schemaEnhancementEngine.url, {
  pathRewrite: (path) => `/schema${path}`,
}));

// User Management Service  →  /auth/*, /users/*, /template* (preserve prefixes)
app.use('/auth',     proxy(SERVICES.userManagementService.url, { pathRewrite: (path) => `/auth${path}` }));
app.use('/users',    proxy(SERVICES.userManagementService.url, { pathRewrite: (path) => `/users${path}` }));
app.use('/template', proxy(SERVICES.userManagementService.url, { pathRewrite: (path) => `/template${path}` }));
