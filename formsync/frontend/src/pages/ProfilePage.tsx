...
import {
  FileJson2, Palette, LogOut, Trash2, Sparkles, CheckCircle2, Clock,
  ArrowRight, Plus, User, Mail, KeyRound, Pencil, X, Check,
...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    authService.getMyTemplates(token)
  ...
  const handleDeleteSchema = async (id: string) => {
    try { await schemaApi.delete(id); setSchemas((p) => p.filter((s) => s.id !== id)); toast.success('Schema deleted'); }
    catch { toast.error('Failed to delete schema'); }
  };
  const handleDeleteTemplate = async (id: string) => {
    if (!token) return;
    try { await authService.deleteTemplate(token, id); setTemplates((p) => p.filter((t) => t.id !== id)); toast.success('Template deleted'); }
    ...
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="relative z-10 border-b border-purple-100/80 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse 120% 100% at 50% 50%, transparent 40%, white 100%)' }} />
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8">
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                ...
                <span className="flex items-center gap-1.5 text-xs text-neutral-400">
                  ...
                  <span className="font-medium text-neutral-700">{templates.length}</span> templates
                </span>
              </div>
            </motion.div>
          ...
          >
            {[
              { label: 'JSON Schemas', value: schemas.length, icon: <FileJson2 className="w-4 h-4 text-purple-500" />, accent: 'border-purple-100 bg-purple-50/60' },
              { label: 'Templates', value: templates.length, icon: <Palette className="w-4 h-4 text-indigo-500" />, accent: 'border-indigo-100 bg-indigo-50/60' },
              { label: 'Total Assets', value: schemas.length + templates.length, icon: <Sparkles className="w-4 h-4 text-violet-500" />, accent: 'border-violet-100 bg-violet-50/60' },
            ].map((s, i) => (
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
            <div className="bg-white rounded-3xl border border-neutral-200/80 shadow-sm overflow-hidden">
              ...
              <div className="px-5 pt-5 pb-0">
                <h2 className="text-base font-bold text-neutral-900 mb-4">My Library</h2>
                <div className="flex gap-1 bg-neutral-100/80 p-1 rounded-xl w-fit mb-0">
                  {(['schemas', 'templates'] as const).map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      className={`relative flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold rounded-lg transition-all duration-200 ${activeTab === tab
                        ? 'bg-purple-100 text-purple-800 shadow-sm border border-purple-200/60'
                        : 'text-neutral-500 hover:text-neutral-700'}`}>
                      {tab === 'schemas' ? <><FileJson2 className="w-3.5 h-3.5" />JSON Schemas <TabBadge count={schemas.length} active={activeTab === tab} /></>
                        : <><Palette className="w-3.5 h-3.5" />Templates <TabBadge count={templates.length} active={activeTab === tab} /></>}
                    </button>
                  ))}
                </div>
              </div>
                  ...
                  {activeTab === 'schemas' ? (
                    <motion.div key="schemas" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
                      {loadingSchemas ? <SkeletonGrid /> : schemas.length === 0 ? (
                        <EmptyState icon={<FileJson2 className="w-9 h-9 text-neutral-300" />} title="No schemas yet"
                          description="Create a schema in the editor, run AI Enhancement and save it."
                          action={<Link to="/editor" className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl text-sm font-semibold hover:shadow-lg hover:shadow-purple-200 transition-all hover:-translate-y-0.5"><Plus className="w-4 h-4" />New Schema</Link>} />
                      ) : (
                        <motion.div variants={stagger} initial="hidden" animate="show" className="grid gap-3 sm:grid-cols-2">
                          {schemas.map((s, i) => <motion.div key={s.id} custom={i} variants={fadeUp}><SchemaCard schema={s} onDelete={() => handleDeleteSchema(s.id)} /></motion.div>)}
                        </motion.div>
                      )}
                    </motion.div>
                  ) : (
                    <motion.div key="templates" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
                      {loadingTemplates ? <SkeletonGrid /> : templates.length === 0 ? (
                        <EmptyState icon={<Palette className="w-9 h-9 text-neutral-300" />} title="No templates saved"
                          description="Build a form in the Form Builder and save it as a template." />
                      ) : (
                        <motion.div variants={stagger} initial="hidden" animate="show" className="grid gap-3 sm:grid-cols-2">
                          {templates.map((t, i) => <motion.div key={t.id} custom={i} variants={fadeUp}><TemplateCard template={t} onDelete={() => handleDeleteTemplate(t.id)} /></motion.div>)}
                        </motion.div>
                      )}
                    </motion.div>
                  )}
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl border border-neutral-200/80 shadow-sm p-6">
        ...
        <ProfileField
          ...
          onEdit={() => { setNameVal(shownName); setEditingField('name'); }}
        ...
        <ProfileField
          ...
          isSaving={savingField}
          onEdit={() => { setEmailVal(shownEmail); setEditingField('email'); }}
          onCancel={() => setEditingField(null)}
            ...
            >
              <div className="pt-4 space-y-3">
                <div className="flex items-center gap-2 text-xs text-neutral-500 mb-1">
                  <KeyRound className="w-3.5 h-3.5" />Change your account password
                </div>
                {/* Current password */}
...
}> = ({ icon, label, value, isEditing, isSaving, onEdit, onCancel, onSave, children }) => (
  <div>
    <div className="flex items-center justify-between mb-1">
      <label className="text-xs text-neutral-400 flex items-center gap-1">{icon}{label}</label>
      {!isEditing && (
        <button onClick={onEdit} className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-0.5 transition-colors">
        ...
        <motion.div key="edit" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.15 }}>
          ...
          <div className="flex gap-2 mt-2">
            <button onClick={onSave} disabled={isSaving}
              ...
              {isSaving ? <span className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin inline-block" /> : <Check className="w-3 h-3" />}
              Save
            </button>
            <button onClick={onCancel} className="flex items-center gap-1 px-3 py-1.5 bg-neutral-100 text-neutral-600 text-xs font-medium rounded-lg hover:bg-neutral-200 transition-colors">
    ...
    <div className="group bg-neutral-50/70 rounded-2xl border border-neutral-200/70 p-4 hover:border-purple-300 hover:bg-white hover:shadow-md hover:shadow-purple-100/60 transition-all duration-200 hover:-translate-y-0.5 flex flex-col">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold ${status.cls}`}>
            {status.icon}{status.label}
          </span>
        </div>
        <button onClick={onDelete} className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-neutral-300 hover:text-red-500 hover:bg-red-50 transition-all" title="Delete">
...
const TemplateCard: React.FC<{ template: any; onDelete: () => void }> = ({ template, onDelete }) => {
  ...
  return (
    <div className="group bg-neutral-50/70 rounded-2xl border border-neutral-200/70 p-4 hover:border-purple-300 hover:bg-white hover:shadow-md hover:shadow-purple-100/60 transition-all duration-200 hover:-translate-y-0.5 flex flex-col">
      ...
      <p className="font-semibold text-sm text-neutral-900 truncate mb-0.5">{template.name}</p>
      {template.description && <p className="text-xs text-neutral-400 line-clamp-1 flex-1">{template.description}</p>}
      <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-neutral-200/60">
        <span className="text-xs text-neutral-400 flex items-center gap-1">
          <Clock className="w-3 h-3" />{new Date(template.createdAt).toLocaleDateString()}
        </span>
        <span className="text-xs px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded-full font-semibold">{fieldCount} {fieldCount === 1 ? 'field' : 'fields'}</span>
      </div>
    </div>
  );
};
    ...
    {[1, 2, 3, 4].map((i) => (
      <div key={i} className="bg-neutral-100 rounded-2xl p-4 animate-pulse">
        ...
        <div className="flex justify-between pt-2 border-t border-neutral-200/60">
          <div className="h-3 w-16 rounded bg-neutral-200" /><div className="h-3 w-10 rounded bg-neutral-200" />
        </div>
      </div>
    ))}
  </div>
