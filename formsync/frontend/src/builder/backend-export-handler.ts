...
export async function exportBackendCode(schemaId: string): Promise<void> {
  try {
    ...
    await generationService.downloadZip(
      validatedSchema,
      schemaData.name?.toLowerCase().replace(/\s+/g, "-") || "backend",
    );
  } catch (error) {
    console.error("[Backend Export] Export failed:", error);
    throw new Error(
      `Failed to export backend code: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
